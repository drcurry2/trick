...
MonteCarloMaster::collate_meta_data()
{
  ...
  for (auto const * var_it : variables) {
    ...
    case MonteCarloVariable::Constant:
      variable_names.push_back (var_it->get_variable_name() + ", Constant, " + var_it->get_assignment()+ ", " + var_it->get_assignment());
      count_const++;
      break;
    ...
    case MonteCarloVariable::Prescribed:
      ...
      count_presc++;
      break;
    case MonteCarloVariable::Uniform:
      {
      char buffer[50];
      sprintf(buffer,", Uniform, %.3e, %.3e",var_it->get_lower_bound(),var_it->get_upper_bound());
      variable_names.push_back (var_it->get_variable_name() + buffer );
      count_rand++;
    ...
    case MonteCarloVariable::Gauss:
      {
      char buffer2[50];
      sprintf(buffer2,", Gauss, %.3e, %.3e",var_it->get_mean(),var_it->get_sigma());
      variable_names.push_back (var_it->get_variable_name() + buffer2 );
      count_rand++;
      std::pair< unsigned int, std::string> var(var_it->get_seed(),
                                                var_it->get_variable_name());
      random_variables.push_back(var);
